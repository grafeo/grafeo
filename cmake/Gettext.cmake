...
IF(XGETTEXT_FOUND)
  MACRO(GETTEXT_CREATE_POT _potFile _pot_options )
    ...
    ADD_CUSTOM_COMMAND(OUTPUT ${_potFile}
        COMMAND ${XGETTEXT_EXECUTABLE} ${_xgettext_options} -o ${_potFile} ${_src_list} --package-name=Grafeo --package-version=0.0.6 --copyright-holder="all rights reserved" --from-code=UTF-8 --msgid-bugs-address=nocturne.pe@gmail.com
  ...
  MACRO(GETTEXT_CREATE_TRANSLATIONS _potFile _firstLang)
    ...
    FOREACH (_currentLang ${_firstLang} ${ARGN})
      ...
      ELSE()
        GET_FILENAME_COMPONENT(_langBasename ${_currentLang} NAME_WE)
        GET_FILENAME_COMPONENT(_langComplete ${_currentLang} NAME)
        STRING(REGEX REPLACE ${_langComplete} ${_langBasename} _currentLang2 ${_currentLang})
        SET(_lang ${_langBasename})
        GET_FILENAME_COMPONENT(_absFile ${_currentLang2}.po ABSOLUTE)
        GET_FILENAME_COMPONENT(_abs_PATH ${_absFile} PATH)
        SET(_gmoFile ${CMAKE_CURRENT_BINARY_DIR}/${_lang}.gmo)
        #MESSAGE("_absFile=${_absFile} _abs_PATH=${_abs_PATH} _lang=${_lang} curr_bin=${CMAKE_CURRENT_BINARY_DIR}")
        if(EXISTS "${_absFile}")
        MESSAGE("existe ${GETTEXT_MSGMERGE_EXECUTABLE} --quiet --update --backup=none -s ${_absFile} ${_absPotFile}")
        ADD_CUSTOM_COMMAND(
            OUTPUT ${_gmoFile}
            ...
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
            COMMENT "Getting main language strings messages ${_absFile}"
            )
        ADD_CUSTOM_TARGET(po_file_merge_${_lang} ALL
            COMMAND ${GETTEXT_MSGMERGE_EXECUTABLE} --quiet --update --backup=none -s ${_absFile} ${_absPotFile}
            COMMAND ${GETTEXT_MSGFMT_EXECUTABLE} -o ${_gmoFile} ${_absFile}
            DEPENDS ${_absPotFile} ${_absFile}
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
        ...
        else(EXISTS "${_absFile}")
        MESSAGE("nao existe ${GETTEXT_MSGINIT_EXECUTABLE} -l ${_langComplete} -o ${_absFile} -i ${_absPotFile}")
        MESSAGE("nao existe ${GETTEXT_MSGFMT_EXECUTABLE} -o ${_gmoFile} ${_absFile}")
        ADD_CUSTOM_COMMAND(
            OUTPUT ${_absFile}
            COMMAND ${GETTEXT_MSGINIT_EXECUTABLE} -l ${_langComplete} -o ${_absFile} -i ${_absPotFile}
            COMMAND ${GETTEXT_MSGFMT_EXECUTABLE} -o ${_gmoFile} ${_absFile}
            DEPENDS ${_absPotFile}
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
            COMMENT "Getting main language strings messages ${_absFile}"
            )
        ADD_CUSTOM_TARGET(po_file_${_lang} ALL
            COMMAND ${GETTEXT_MSGINIT_EXECUTABLE} -l ${_langComplete} -o ${_absFile} -i ${_absPotFile}
            COMMAND ${GETTEXT_MSGFMT_EXECUTABLE} -o ${_gmoFile} ${_absFile}
            DEPENDS ${_absPotFile}
            WORKING_DIRECTORY ${CMAKE_CURRENT_BINARY_DIR}
